...
def main():
    ...
    if datasets_list:
        ...
        print("Number of datasets considered (0 = all):", datasets_num)
        ...
        for dataset_item in datasets_list:
            ...
            dataset_name = Path(dataset_file_name).stem

            ### Check column values from JSON settings ###
            csv_separator = dataset_item.get('separator') or csv_sep # if empty, and assign the default value
            ...
            df_dataset_stats = dataset_stats(df_log, dataset_file_name, case_id_col, outocome_col, activity_col, datasets_stats_dir)
            ...
            low_frequency_cases = activity_frequency[activity_frequency < log_default_threshold].index
            # Find case IDs associated with these low-frequency activities
            cases_to_remove = df_log[df_log[case_id_col].isin(low_frequency_cases)][case_id_col].unique()
            ...
            ### Ordering ###
            df_log.sort_values([case_id_col, timestamp_col], ascending=[True, True], inplace=True)
            ...
            if eventnr_col not in df_log.columns:
                ...
                df_log.sort_values([case_id_col, eventnr_col], ascending=[True, True], inplace=True)
                print()
            else:
                df_log.sort_values([case_id_col, eventnr_col], ascending=[True, True], inplace=True)
            ...
            ### Prefixes ###
            print("> Prefixes")
            print("Prefix list for this dataset:", prefix_list)
            sum_sublog = 0
            ...
            if task_script["ENCODING"] == 1:
                ...
                if encoding_type == "I":
                    ...
                    df_log_encoded = index_encoding(df_log, case_id_col, activity_col, eventnr_col, outocome_col)
            ...
            if task_script["PREDICTION"] == 1:
                ...
                for key, value in task_ml.items(): # key = algorithm name, value = 1/0 (yes/no)
                    if value == 1:
                        ...
                        metrics_model = ml_xgboost(X_train, X_test, y_train, y_test, outcome_column=outocome_col)
                        print("- Metrics")
                        print(metrics_model)
                        ml_save_metrics(metrics_model, dataset_name_enc, ml_results_dir, key)
                print("-"*8)
...
if __name__ == "__main__":
    # encoded_traces_padded, labels = main(file_path)
    main()